...
As of the Phase 8 Step 0 investigation (2026-09-13), both installed CLIs *do* report exact, real-time plan-usage percentages directly in their own structured output — Claude's `rate_limit_event` message, Codex's `token_count` event's `rate_limits` object (see `packages/agents/src/usage-extraction.ts`'s `extractRateLimitReadings`, and the completion record in `IMPLEMENTATION_STATUS.md` for exactly what was confirmed live versus from historical session logs). `recordCliReportedUsage(provider, readings)` writes these as `CLI_REPORTED`/`EXACT` rows — the same table, no special-casing in `evaluate`/`getProviderUsage`, since Claude's `five_hour`/`seven_day` and Codex's `primary`/`secondary` windows map directly onto the pre-existing `"5H"`/`"WEEKLY"` windowId vocabulary. `refresh()` still reports honestly that there's no on-demand poll — only automatic updates after a run completes. The other two data sources remain: `submitManualSnapshot` (validated, always labeled `MANUAL`) and `recordRateLimitError`, which heuristically pattern-matches a provider process's stderr/failure text (`parseRateLimitMessage`) for wording specific to the *account/subscription usage allowance* ("usage limit reached", "5-hour limit", "usage cap", "quota/allowance exceeded") plus a nearby reset-time phrase. It explicitly excludes wording that indicates an API-level tokens/requests-per-minute throttle (`tokens per minute`, `requests per minute`, `TPM`/`RPM`, `rate_limit_error`, `429`) — conflating that with the subscription usage window is exactly the mistake this project must avoid.
...
`packages/agents/src/usage-extraction.ts`'s `extractTokenUsage(provider, value)` is the provider-neutral, pure counterpart to `extractRateLimitReadings` above — Claude: from a `type: "result"` event's `usage`/`output_tokens_details`/`total_cost_usd`; Codex: from a usage-shaped object (`input_tokens`/`cached_input_tokens`/`cache_write_input_tokens`/`output_tokens`/`reasoning_output_tokens`/`total_tokens`). Both search defensively for the recognizable data shape rather than requiring one exact event envelope — Codex's exact wrapping inside `codex exec --json` specifically was not empirically confirmed (its usage limit was exhausted during the investigation; the shapes come from its real local session logs instead), so this tolerates a wrapping difference until that's confirmed for certain.
