...
As of the Phase 8 Step 0 investigation (2026-09-13), Claude Code *does* report exact, real-time plan-usage percentages directly in its own structured output — a `rate_limit_event` message with `rate_limit_info.unifiedWindows.{five_hour,seven_day}` (see `packages/agents/src/usage-extraction.ts`'s `extractRateLimitReadings`). `recordCliReportedUsage(provider, readings)` writes these as `CLI_REPORTED`/`EXACT` rows — the same table, no special-casing in `evaluate`/`getProviderUsage`, since Claude's `five_hour`/`seven_day` windows map directly onto the pre-existing `"5H"`/`"WEEKLY"` windowId vocabulary. **Codex does not expose the equivalent through the invocation path this app drives**: a real `codex exec --json` completion (run once Codex's own usage limit reset, confirming this for certain rather than leaving it as a shape-based guess) showed a `turn.completed` event with token usage but no rate-limit reading at all; `codex doctor --json` has no rate-limit/usage check either. Codex's *interactive session* telemetry does carry an equivalent `rate_limits` object (confirmed from real local session logs), so `extractRateLimitReadings`'s Codex branch is kept — harmless, and ready if a future CLI version starts including it in `exec`'s own stream — but it will reliably return `[]` for anything this app invokes today. `refresh()` still reports honestly that there's no on-demand poll — only automatic updates after a run completes, and only for a provider that actually reports it. The other two data sources remain: `submitManualSnapshot` (validated, always labeled `MANUAL`) and `recordRateLimitError`, which heuristically pattern-matches a provider process's stderr/failure text (`parseRateLimitMessage`) for wording specific to the *account/subscription usage allowance* ("usage limit reached", "5-hour limit", "usage cap", "quota/allowance exceeded") plus a nearby reset-time phrase. It explicitly excludes wording that indicates an API-level tokens/requests-per-minute throttle (`tokens per minute`, `requests per minute`, `TPM`/`RPM`, `rate_limit_error`, `429`) — conflating that with the subscription usage window is exactly the mistake this project must avoid.
...
- `AgentRunManager` accepts an optional `UsageSafetyService` and calls `recordRateLimitError` on every `stderr` chunk and `failed` event, and (new in Phase 8) `recordCliReportedUsage` on every `structured_output` event that yields a rate-limit reading — independent of the preflight checks above.
...
## Per-run token capture and historical backfill (Phase 8, first slice)

`usage_records` (`apps/server/src/db/schema.ts`) is one row per agent run, always — `usageSource: "unavailable"` with every token field `null` when nothing was recoverable, so "exactly one usage record per run" is an invariant later aggregation work can rely on. Deliberately smaller than `USAGE_MONITORING_SPEC.md`'s full `UsageRecord`: no cost fields yet (no pricing registry exists), and `role` reuses `agentRuns.role`'s own enum rather than the spec's example vocabulary — this app's actual workflow roles already serve as `workflowType`.
...
`GET /api/agent-runs/:id` returns a `usage` field; `apps/web/src/App.vue`'s run-detail view shows a compact token line. No dashboard, cost figure, or budget UI exists yet — see `IMPLEMENTATION_STATUS.md`'s Phase 8 record for what's deferred and why.
