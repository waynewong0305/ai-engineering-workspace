...
Every `QUESTION` record gets its own **QUESTIONS** card on the task detail pane (below the evidence board's add-record form), separate from the generic Fact/Assumption/Decision/Experiment-result list. Use the **Open / Answered / Deferred / Duplicates / All records** tabs to filter. Each card shows why it matters, a suggested next step, and any AI-suggested candidate answers whenever the analysis or cross-review that raised the question supplied them — this happens automatically as part of that same run, spending no extra provider usage; a question raised before this existed, or one you add yourself, stays blank there. Cards offer **Write answer**, **Defer**, **Not applicable**, and **Mark as duplicate** while a question is open — or **Reopen** (or **Remove duplicate link**, for a confirmed duplicate) once it's resolved. When you write an answer, if AI-suggested candidate answers exist you can pick one directly or choose **Write my own answer** instead; either way the answer is saved permanently, and every past answer, deferral, or status change stays visible under each card's **History** disclosure — nothing is ever overwritten, only added to.

Two more ways to catch duplicate questions sit just below the filter tabs. **Group exact duplicates** is free and instant — it finds open questions that are the same once case, punctuation, and extra spacing are ignored, keeps the earliest one as the canonical, and links the rest to it (reporting honestly if it finds none, rather than doing nothing silently). Real duplicate questions are often worded quite differently, though, so **Find possible duplicates (AI)** asks your chosen provider to read every open question and flag ones asking substantively the same thing — like the free synthesis and plan-revision actions, this spends provider usage and is gated by the same usage-safety check. Once it finishes, any question it flagged shows a **Possible duplicate of: `<question text>`** hint with **Confirm** (links it, same as **Mark as duplicate**) and **Dismiss** (hides the suggestion for that question only — it doesn't change anything stored, so re-running the scan later can still surface it again).

Any open question raised before this project could generate why-it-matters/suggested-action content — or one you typed in yourself — stays blank there forever unless you fill it in. **Generate missing suggestions (N)**, shown only when at least one open question qualifies, asks your chosen provider to write that content for every currently-blank question at once. This spends a small amount of provider usage and only ever produces a preview — nothing is saved yet. Each suggested question appears as an editable card (priority, why it matters, suggested action, evidence needed, and candidate answers, one per line) so you can correct anything before it's kept; **Accept** saves one card as shown, **Accept all** saves every card at once, and **Discard** drops a card from the preview without saving it or changing anything about the question. Accepting never overwrites a question that already has suggestions, so it's safe to re-run later — anything already filled in is simply skipped.
...
## Frontend verification and review approval — planned
...
guarantee that the UI still looks or behaves correctly; you must currently exercise consequential UI
changes yourself before merging.
...
should assess the UI/UX, the workspace will first ask you for a separate approval — showing why
review is recommended, which provider/agent configuration would run, which pages/scenarios/evidence
would be shared, and that the run may consume provider usage — before anything is shared with
Claude, Codex, or any future provider. Starting a build, allowing web access, or approving an
ordinary code review will never count as this approval; declining will report that agent UI review
was skipped or that human review is required, never that the UI was verified.

The server-side enforcement for this approval already exists (`FrontendReviewApprovalService`) and
is fully tested, but there is no UI for it yet and nothing in the workspace calls it, because the
browser-verification runner that would actually create these requests hasn't been built. Rather than
ship a form that makes you hand-author the disclosure a runner is meant to generate, the approval
screen stays hidden until the runner exists to drive it automatically.
...
Claude Code reports exact, current plan usage in its run output; the workspace records it as **CLI_REPORTED · EXACT**. Codex's run output contains token counts but not plan percentages, so the workspace asks Codex's documented local App Server for the current ChatGPT plan windows and records them as **APP_SERVER · EXACT**. This read does not start a model turn or redeem a reset credit.

- **Refresh** for Codex asks Codex App Server for a current reading — free, instant, and this is also done automatically before every provider-consuming action. Claude has no equivalent free endpoint, so its Refresh button behaves differently: clicking it asks you to confirm first, then sends one minimal request to Claude's cheapest model just to read your current usage, which uses a small amount of real Claude usage. Because of that cost, the workspace never does this automatically on your behalf — only this manual click does; the rest of the time, Claude's reading still updates for free as a byproduct of your normal runs. If a provider has no supported on-demand reader at all, the button reports that honestly without changing the existing data.
