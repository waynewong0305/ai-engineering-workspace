...
- Automatic detection only catches wording it recognizes. If a run or brainstorm task fails and you suspect it was actually a usage limit that wasn't recognized, a **Mark \<provider\> as exhausted** button appears next to the failure. Selecting it records the same kind of manual snapshot (100% used, labeled MANUAL) so the next provider call is blocked instead of failing the same way again.
