...
- **Refresh** asks Codex App Server for a current Codex reading. Claude continues to update from provider run output; if a provider has no supported on-demand reader, the button reports that honestly without changing the existing data.
