...
- Known limitation: the rate-limit-error parser matches on plausible historical provider wording (e.g. "usage limit reached", "5-hour limit", "rate limit reached") and a nearby reset-time phrase; it has not been validated against a real exhausted response (doing so would consume real usage, which this task must not do) and should be revisited once real refusal text is observed.
